<g:hiddenField name="declaracionJuradaId" value="${declaracionJuradaInstance?.declaracionJuradaId}" />
<g:hiddenField name="id" value="${declaracionJuradaInstance?.declaracionJuradaId}" />
<g:hiddenField name="version" value="${declaracionJuradaInstance?.version}" />

<div style="display: none;">
	<div id="urlGetCuentas">
		<g:createLink controller="cuenta" action="ajaxGetCuentasNombresSQL" />
	</div>
	<div id="urlDownloadDDJJ">
...
<div class="form-group row">
	...
	<div class="col-sm-10">
		<input id="fecha" name="fecha" class="form-control" type="text" data-format="d/m/Y" placeholder="Seleccione un día" value="${declaracionJuradaInstance?.fecha?.toString('dd/MM/yyyy')}"/>
	</div>
</div>
...
<div class="form-group row">
	...
	<div class="col-sm-10">
		<textarea class="form-control" id="descripcion" name="descripcion" rows="3" class="form-control ${hasErrors(bean: declaracionJuradaInstance, field: 'descripcion', 'form-control-danger')}">${declaracionJuradaInstance?.descripcion}</textarea>
	</div>
</div>
	...
	<div class="form-group row">
		...
		<div class="col-sm-10">
			<div class="input-group" onclick="">
				<span class="btn btn-primary" onclick="downloadDDJJ()">${declaracionJuradaInstance?.nombreArchivo} <i class="icofont icofont-download-alt"></i> </span>
			</div>
		</div>
	</div>
</g:if>
...
$(document).ready(function () {
	...
	/*** ESTADOS ***/
	$("#cbEstado").select2({
		placeholder: '<g:message code="zifras.documento.declaracionJurada.estado.placeHolder" default="Seleccione un estado"/>',
	...
	llenarCombo({
		comboId : "cbEstado",
		ajaxLink : '${createLink(controller:"estado", action:"ajaxGetDeclaracionJuradaEstados")}',
		idDefault : '${declaracionJuradaInstance?.estadoId}'
	});
	...
	llenarCombo({
		...
		idDefault : '${declaracionJuradaInstance?.cuentaId}',
		atributo : 'toString',
...
function downloadDDJJ(){
	window.location.href = $('#urlDownloadDDJJ').text() + '/' + "${declaracionJuradaInstance?.declaracionJuradaId}";
}
</script>
