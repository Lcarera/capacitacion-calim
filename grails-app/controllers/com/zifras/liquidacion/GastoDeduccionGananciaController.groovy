...
class TipoGastoDeduccionGananciaController {
	...
	def save(TipoGastoDeduccionGananciaCommand command) {
		...
		try {
			...
			flash.error	= message(code: 'zifras.TipoGastoDeduccionGanancia.save.error', default: 'Error al intentar salvar la Gasto/Deduccion de Ganancia')
			render(view: "create", model: [tipoGastoDeduccionGananciaInstance: command])
			return
		}
	...
	def show(Long id) {
		...
		if (!tipoGastoDeduccionGananciaInstance) {
			flash.error = message(code: 'default.not.found.message', args: [message(code: 'zifras.TipoGastoDeduccionGanancia.label', default: 'TipoGastoDeduccionGanancia'), id])
			redirect(action: "list")
			return
		}
	...
	def delete(Long id) {
		def tipoGastoDeduccionGananciaInstance = tipoGastoDeduccionGananciaService.getTipoGastoDeduccionGanancia(id)
		if (!tipoGastoDeduccionGananciaInstance) {
			flash.message = message(code: 'default.not.found.message', args: [message(code: 'zifras.TipoGastoDeduccionGanancia.label', default: 'TipoGastoDeduccionGanancia'), id])
			redirect(action: "list")
			return
		}
